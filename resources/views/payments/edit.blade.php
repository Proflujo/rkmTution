                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ url('/payments/user/store') }}">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <input id="account_id" type="hidden" class="form-control" name="account_id" value="{{ old('account_id') }}{{$paymentDetails->account_id}}" autocomplete="ammount">
                                </div>
                            </div>
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <select name="username" id="username" class="form-select @error('username') is-invalid @enderror selectpicker" aria-label="Default select example">
                                        ...
                                        @foreach($users as $user)
                                            <option value="{{$user->id}}" {{$user->id==$paymentDetails->fkuser_id? 'selected' : ''}}>{{$user->username}}</option>
                                        @endforeach
                                    </select>
                                    @error('username')
                                ...
                                <div class="col-md-6">
                                    <select name="branch" id="branch" class="form-select @error('branch') is-invalid @enderror selectpicker" aria-label="Default select example">
                                        ...
                                        @foreach($branchs as $branch)
                                            <option value="{{$branch->value}}" {{$branch->value == $paymentDetails->branchValue ? 'selected':''}}>{{$branch->branch}}</option>
                                        @endforeach
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <input id="ammount" type="ammount" class="form-control @error('ammount') is-invalid @enderror" name="ammount" value="{{ old('ammount') }}{{$paymentDetails->payment}}" autocomplete="ammount">
