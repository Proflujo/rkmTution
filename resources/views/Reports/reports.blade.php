...
@section('content')
    <div class="custom-datatable">
        <div class="row justify-content-center">
            <div class="col-md-11">
                <div class="card">
                    <div class="card-header text-black">{{ __('Monthly Report') }}                    
                    ...
                    <div class="card-body">  
                        ...
                        <form id="filterForm">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                	<label for="month" class="form-label text-md-end">{{ ('Choose Month') }}</label>
    	                            <select name="month" id="month" class="form-select @error('month') is-invalid @enderror selectpicker" aria-label="Default select example">
                                        <?php 
                                            $currMonth = (int)date('m') ;
                                        ?>
                                    @foreach($months as $month)
                                        <option value="{{ $month->monthId}}" {{ $month->monthId == $currMonth ? 'selected' : ''}} >{{ $month->monthName}}</option>
                                    @endforeach   
                                    </select>
                        ...
                        <div class="col-md-12">
                            <table id="reportsList" class="table table-bordered table-striped">
...
@section('bottom-scripts')
   ...
   $(document).on('click', '#filter', function(e){
        e.preventDefault();
        $('#reportsList').DataTable().clear().destroy();
        year = $('#year').val();
        month = $('#month').val();
        getMonthandYear(year,month);
